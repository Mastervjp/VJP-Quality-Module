...
export class OperationService {
  ...
  updatestatus(id, status): Observable<any> {
    ...
    return this.http.put<any>(this.API_URL + '/api/operation/' + id, status, { headers: { Authorization: `Bearer ${this.getToken()}` } }).pipe(
      map(this.extractData1));
  ...
  updatemasterstatus(id, masterApproval): Observable<any> {
    ...
    return this.http.put<any>(this.API_URL + '/api/operation/' + id, masterApproval, { headers: { Authorization: `Bearer ${this.getToken()}` } }).pipe(
      map(this.extractData1));
  ...
  approval1(opnNo, qpMasterApproval): Observable<any> {
