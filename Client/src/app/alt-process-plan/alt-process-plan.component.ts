...
@Component({
  selector: 'app-alt-process-plan',
  templateUrl: './alt-process-plan.component.html',
  styleUrls: ['./alt-process-plan.component.css']
})
export class AltProcessPlanComponent implements OnInit {
  ...
  callOperation(product) {
    ...
    this.router.navigate(['/altquality']); //we can send product object as route param
  }
  ...
  getoperation() {
    ...
    let drgid = localStorage.getItem('DrgCode');
    this._operationservice.getAltprocess(drgid).subscribe((res: any) => {
      if (res.success) {
        let mydata = res.data;
